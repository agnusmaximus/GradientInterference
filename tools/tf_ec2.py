...
cfg_resnet = Cfg({
    "name" : "gradient_interference_resnet_test",
    "key_name": "MaxLamKeyPair3",          # Necessary to ssh into created instances
    ...
    "path_to_keyfile" : "/Users/maxlam/Desktop/School/Fall2016/Research/DistributedSGD/MaxLamKeyPair3.pem",
...
cfg_resnet_cifar100 = Cfg({
    "name" : "gradient_interference_resnet_cifar100_test",
    "key_name": "MaxLamKeyPair3",          # Necessary to ssh into created instances
...
cfg_mnist = Cfg({
    "name" : "gradient_interference_mnist_test",
    "key_name": "MaxLamKeyPair3",          # Necessary to ssh into created instances
   ...
   #"nfs_ip_address" : "172.31.30.114",         # us-west-2b
    ...
    [
        # Evaluation command
        "python mnist/src/mnist_eval.py "
        "--eval_dir=%(base_out_dir)s/eval_dir "
        "--batch_size=1 "
        "--checkpoint_dir=%(base_out_dir)s/train_dir "
        "> %(base_out_dir)s/out_evaluator 2>&1 &",
...
cfg_convnet_single = Cfg({
    "name" : "gradient_interference_cudaconvnet_single_test",
    "key_name": "MaxLamKeyPair3",          # Necessary to ssh into created instances
...
cfg_resnet_single = Cfg({
    "name" : "gradient_interference_cudaconvnet_resnet_single_test",
    "key_name": "MaxLamKeyPair3",          # Necessary to ssh into created instances
   ...
   #"nfs_ip_address" : "172.31.30.114",         # us-west-2b
    ...
    "train_commands" :
    [
        "python resnet/src/resnet_single_train.py "
        "--batch_size=%(batch_size)s "
        "--dataset=cifar10 "
        "--learning_rate=%(learning_rate)s "
        "--replicate_data_in_full=false "
...
cfg_dcgan = Cfg({
    "name" : "gradient_interference_dcgan_test",
    "key_name": "MaxLamKeyPair3",          # Necessary to ssh into created instances
   ...
   #"nfs_ip_address" : "172.31.30.114",         # us-west-2b
    ...
    [
        "cd GradientInterference",
        "git fetch && git reset --hard origin/master",
       "python dcgan/src/download.py mnist",
    ],
    ...
    "train_commands" :
    [
        "python dcgan/src/dcgan_distributed_train.py "
        "--input_height 28 "
        "--output_height 28 "
        "--input_width 28 "
        "--output_width 28 "
    ...
    # Commands to run on the evaluator
    # Currently for dcgan there is no evaluation code, so this does nothing.
    "evaluate_commands" :
    [
        # Evaluation command
        "python resnet/src/resnet_eval.py "
